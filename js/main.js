...
$(function () {
  ...
  function isAdjust(n) {
    ...
    if (t) headerAdjust();else headerAdjustBack();
  }
  ...
  function initAjust() {
    if (window.innerWidth < 768) headerAdjust();else isAdjust(2);
  }
  ...
  function openSidebar() {
    ...
    var isAdjustTimeCount = window.setInterval(function () {
      if ($pageHead.hasClass('fixed')) isAdjust(1);else isAdjust(2);
    }, 100);
    setTimeout(function () {
  ...
  function openMobileSidebar(name) {
    ...
    if (name === 'menu') {
      ...
      for (var i = 0; i <= $mobileSidevarMenusChild.length; i++) {
        var duration = i / 5 + 0.2;
  ...
  var $figureHighlight = $('figure.highlight');

  if ($figureHighlight.length) {
    var isHighlightCopy = GLOBAL_CONFIG.highlightCopy;
    var isHighlightLang = GLOBAL_CONFIG.highlightLang;
    var isHighlightShrink = GLOBAL_CONFIG.highlightShrink;
    ...
    */
    ...
    var $highlightTools = $('.highlight-tools');
        ...
        if (document.queryCommandSupported && document.queryCommandSupported('copy')) {
          try {
            ...
            if (isSnackbar) {
              ...
              $(ctx).prev('.copy-notice').text(GLOBAL_CONFIG.copy.success).animate({
                opacity: 1,
                right: 30
              }, 450, function () {
                setTimeout(function () {
            ...
            if (isSnackbar) {
              ...
              $(ctx).prev('.copy-notice').text(GLOBAL_CONFIG.copy.error).animate({
                opacity: 1,
                right: 30
              }, 650, function () {
                setTimeout(function () {
  ...
  if ($justifiedGallery.length) {
    ...
    var initJustifiedGallery = function initJustifiedGallery(selector) {
  ...
  if (GLOBAL_CONFIG_SITE.isPost && GLOBAL_CONFIG_SITE.isSidebar) {
    ...
    var expandToc = function expandToc($item) {
      if ($item.is(':visible')) {
        return;
      }
    ...
    var findHeadPosition = function findHeadPosition(top) {
      ...
      if (currentId && currentActive.attr('href') !== currentId) {
        ...
        var parents = _this.parents('.toc-child'); // Returned list is in reverse order of the DOM elements
        ...
        topLink // Find all top-level .toc-item containers, i.e. sections
        // excluding the currently active one
        .closest('.toc-item').siblings('.toc-item') // Hide their respective list of subsections
        .find('.toc-child').hide();
      }
    };

    var autoScrollToc = function autoScrollToc(currentTop) {
  ...
  function changeLightIcon() {
    try {
      if (!['/blhx/', '/hqxy/', '/dal/', '/bh3/'].includes(location.pathname)) loadModel(105, 1);
    } catch (e) {}
  ...
  function changeDarkIcon() {
    try {
      if (!['/blhx/', '/hqxy/', '/dal/', '/bh3/'].includes(location.pathname)) loadModel(131, 1);
    } catch (e) {}
