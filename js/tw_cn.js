...
var translateButtonObject,
    ...
    Snackbar_tl = GLOBAL_CONFIG.Snackbar,
    defaultEncoding = translate.defaultEncoding,
    translateDelay = translate.translateDelay,
    cookieDomain = translate.cookieDomain,
    msgToTraditionalChinese = translate.msgToTraditionalChinese,
    msgToSimplifiedChinese = translate.msgToSimplifiedChinese,
    translateButtonId = "translateLink",
    currentEncoding = defaultEncoding,
    targetEncodingCookie = "targetEncoding" + cookieDomain.replace(/\./g, ""),
    targetEncoding = null == Cookies.get(targetEncodingCookie) ? defaultEncoding : Cookies.get(targetEncodingCookie),
    isSnackbar = void 0 !== GLOBAL_CONFIG.Snackbar;

function translateText(t) {
  return "" == t || null == t ? "" : 1 == currentEncoding && 2 == targetEncoding ? Simplized(t) : 2 == currentEncoding && 1 == targetEncoding ? Traditionalized(t) : t;
}

function translateBody(t) {
  if ("object" == _typeof(t)) var n = t.childNodes;else n = document.body.childNodes;

  for (var e = 0; e < n.length; e++) {
    var a = n.item(e);
    0 < "||BR|HR|TEXTAREA|".indexOf("|" + a.tagName + "|") || a == translateButtonObject || ("" != a.title && null != a.title && (a.title = translateText(a.title)), "" != a.alt && null != a.alt && (a.alt = translateText(a.alt)), "INPUT" == a.tagName && "" != a.value && "text" != a.type && "hidden" != a.type && (a.value = translateText(a.value)), 3 == a.nodeType ? a.data = translateText(a.data) : translateBody(a));
  }
}

function translatePage() {
  1 == targetEncoding ? (currentEncoding = 1, targetEncoding = 2, translateButtonObject.innerHTML = msgToTraditionalChinese, Cookies.set(targetEncodingCookie, targetEncoding, {
    expires: 7,
    path: "/"
  }), translateBody(), isSnackbar && snackbarShow(Snackbar_tl.cht_to_chs)) : 2 == targetEncoding && (currentEncoding = 2, targetEncoding = 1, translateButtonObject.innerHTML = msgToSimplifiedChinese, Cookies.set(targetEncodingCookie, targetEncoding, {
    expires: 7,
    path: "/"
  }), translateBody(), isSnackbar && snackbarShow(Snackbar_tl.chs_to_cht));
}
...
function Traditionalized(t) {
  for (var n = "", e = JTPYStr(), a = FTPYStr(), r = 0; r < t.length; r++) {
    1e4 < t.charCodeAt(r) && -1 != e.indexOf(t.charAt(r)) ? n += a.charAt(e.indexOf(t.charAt(r))) : n += t.charAt(r);
  }

  return n;
}

function Simplized(t) {
  for (var n = "", e = JTPYStr(), a = FTPYStr(), r = 0; r < t.length; r++) {
    1e4 < t.charCodeAt(r) && -1 != a.indexOf(t.charAt(r)) ? n += e.charAt(a.indexOf(t.charAt(r))) : n += t.charAt(r);
  }
...
function translateInitilization() {
  (translateButtonObject = document.getElementById(translateButtonId)) && ("object" != _typeof(document.all) ? translateButtonObject.href = "javascript:translatePage();" : (translateButtonObject.href = "#", translateButtonObject.onclick = new Function("translatePage(); return false;")), currentEncoding != targetEncoding && (setTimeout("translateBody()", translateDelay), translateButtonObject.innerHTML = 1 == targetEncoding ? msgToSimplifiedChinese : msgToTraditionalChinese));
}
