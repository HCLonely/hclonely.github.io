...
window.live2d_settings = Array(); // 后端接口
...
function initModel(waifuPath, type) {
  ...
  if (live2d_settings.waifuEdgeSide[0] == 'left') $(".waifu").css("left", live2d_settings.waifuEdgeSide[1] + 'px');else if (live2d_settings.waifuEdgeSide[0] == 'right') $(".waifu").css("right", live2d_settings.waifuEdgeSide[1] + 'px');
  ...
  if (_typeof(waifuPath) == "object") loadTipsMessage(waifuPath);else {
    $.ajax({
      cache: true,
...
function loadModel(modelId) {
  var modelTexturesId = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
...
function loadTipsMessage(result) {
  ...
  $('a.sns').on('mouseover', function () {
    var text = $(this).parent().text().trim().replace(/：[\w\W]*/g, "");
  ...
  window.showWelcomeMessage = function (result) {
    ...
    if (window.location.href == live2d_settings.homePageUrl) {
      ...
      if (now > 23 || now <= 5) text = getRandText(result.waifu.hour_tips['t23-5']);else if (now > 5 && now <= 7) text = getRandText(result.waifu.hour_tips['t5-7']);else if (now > 7 && now <= 11) text = getRandText(result.waifu.hour_tips['t7-11']);else if (now > 11 && now <= 14) text = getRandText(result.waifu.hour_tips['t11-14']);else if (now > 14 && now <= 17) text = getRandText(result.waifu.hour_tips['t14-17']);else if (now > 17 && now <= 19) text = getRandText(result.waifu.hour_tips['t17-19']);else if (now > 19 && now <= 21) text = getRandText(result.waifu.hour_tips['t19-21']);else if (now > 21 && now <= 23) text = getRandText(result.waifu.hour_tips['t21-23']);else text = getRandText(result.waifu.hour_tips["default"]);
    } else {
  ...
  if (live2d_settings.showHitokoto) {
    ...
    window.hitokotoTimer = 30000;
    window.hitokotoInterval = true;
