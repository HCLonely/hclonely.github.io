...
!function (z) {
  function a(t, e, a) {
    ...
    function Y(t) {
      t && (S = t.getFullYear(), A = t.getMonth(), i());
    }
    ...
    function $() {
      ...
      }), null !== s && d() && (E.posts = s[S + "-" + (A + 1)])) : function () {
        null === o && z.ajax({
    ...
    function d() {
      ...
      if (null !== o && 0 !== o.length && (0 !== E.posts.length || !(null === E.prev && null !== E.next && E.next.getTime() > t || null === E.next && null !== E.prev && E.prev.getTime() < t))) {
        E.posts = [];

        for (var e = 0; e < o.length; e++) {
          var a = o[e].getTime();
          if (t === a) return E.prev = 0 === e ? null : o[e - 1], E.next = e === o.length - 1 ? null : o[e + 1], 1;
    ...
    function i() {
      ...
      t <= 0 && (t = 6 - -1 * (t + 1));
      var e = new Date(S, A + 1, 0).getDate(),
          a = new Date(S, A, 0).getDate() - t + 1,
          n = z("<div/>").addClass("cal-head"),
          r = z("<div/>"),
          l = z("<div/>"),
          s = z("<div/>").addClass("cal-title");
      ...
      for (var o = z("<table/>").addClass("cal"), u = R.weekOffset, p = z("<thead/>"), d = z("<tr/>"), i = 0; i < 7; i++) {
        6 < u && (u = 0);
        var c = z("<th/>").attr("scope", "col").attr("title", R.dayOfWeek[u]);
        c.html(R.dayOfWeekShort[u]), d.append(c), u++;
      }

      p.append(d), o.append(p);
      var f = z("<tfoot/>"),
          h = z("<tr/>"),
          y = z("<td/>").attr("colspan", 3),
          g = z("<td/>").html("&nbsp;"),
          v = z("<td/>").attr("colspan", 3);
      ...
      for (i = 0; i < 6; i++) {
        for (var D = z("<tr/>"), k = 0; k < 7; k++) {
          var w = z("<td/>");
          if (7 * i + k < t) w.addClass("cal-gray"), w.html(a++);else if (M <= e) {
            ...
            if (0 !== C.num) {
              var b = C.keys[0],
                  L = z("<a>").attr("href", E.posts[b].link).attr("title", E.posts[b].title).html(M++);
              w.append(L);
            } else w.html(M++);
          } else w.addClass("cal-gray"), w.html(x++);
          D.append(w);
        }
    ...
    return "/" !== R.root[0] && (R.root = "/" + R.root), "/" !== R.root[R.root.length - 1] && (R.root += "/"), i();
  }
